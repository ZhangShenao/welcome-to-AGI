...
以下是一个完整的五子棋游戏实现，所有代码都包含在一个 HTML 文件中。这个实现包含基本的五子棋功能：绘制棋盘、轮流下棋、判断胜负等。
...
1. **棋盘绘制**：15×15的标准五子棋棋盘，包含天元和星位标记
2. **轮流下棋**：黑棋先行，然后白棋，交替进行
3. **胜负判断**：当一方连成五子时（横、竖、斜方向），游戏结束并显示获胜方
4. **重新开始**：可以随时重新开始游戏
5. **响应式设计**：适配不同屏幕尺寸
